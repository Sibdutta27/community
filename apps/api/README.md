...
- Build Command: `pnpm install --frozen-lockfile && pnpm --filter yucayekeconnect-server build`
  - ensure `prisma generate` runs (Turbo wires `db:generate` before `build`, or add a
    `postinstall`/explicit `pnpm --filter yucayekeconnect-server db:generate`).
- Start Command: `pnpm --filter yucayekeconnect-server start:render`
